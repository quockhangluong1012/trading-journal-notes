...
- **Hiện tại:** 17 setup (13 Win / 4 Loss ≈ 76%, n=17 còn nhỏ)
...
| 2026-07-15 |        30 | **1 setup 2022-model mới — Loss chất lượng cao (bài học POI Priority).** `04 - Backtesting/2026-07-15/01 - Loss - XAUUSD - 2014-05-19 - Short` (**XAUUSD Short, Loss, Grade C+, r_planned 3.9 → r_multiple −1, followed_plan Yes**). Chuỗi Sweep→Displacement→MSS→FVG→Entry đủ 7 bước trên M1 và entry đúng CE M1 FVG (kỷ luật tốt), nhưng **bước 0 sai — POI selection**: chọn Breaker Block nông (0.618) thay vì Order Block sâu (0.786) terminal → SL rơi trong đường thanh khoản dẫn tới OB → bị quét ("loss đúng hướng": giá chạm TP SAU khi quét SL). Tổng **17/100 = 17%**. Ràng buộc THẬT vẫn là **win rate >55%**: mẫu nay **13 Win / 4 Loss ≈ 76%** (giảm từ 81% do +1 Loss), **n=17 vẫn quá nhỏ**. Prose "Hiện tại" sync 16 → **17 setup**. ⭐ Rút ra **Rule POI Priority** + **Rule SL theo POI terminal** (không siết SL cho đẹp RR — nếu SL đúng thì RR sụp còn ~0.5R = tín hiệu bỏ lệnh) → nạp vào playbook. **Cột mốc #1 (viết checklist vào/ra lệnh thành văn) vẫn 0/5 — đòn bẩy prep bị hoãn lâu nhất, không cần lệnh live để làm.** |
